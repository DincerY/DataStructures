...
{
    public bool IsEndOfWord { get; set; }
    public Dictionary<char, DenemeTrie> ChilderenTries { get; set; } = new();

    public void Add(string word)
    {
        var currentDenemeTrie = this;

        foreach (var c in word)
        {
            if (!currentDenemeTrie.ChilderenTries.TryGetValue(c,out var denemeTrie))
            {
                denemeTrie = new();
                currentDenemeTrie.ChilderenTries.Add(c, denemeTrie);
            }
            currentDenemeTrie = denemeTrie;
        }
        currentDenemeTrie.IsEndOfWord = true;
